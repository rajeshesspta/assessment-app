...
- Tenant Admin provisions tenants, rotates API keys, and manages cohorts + access policies. They create Content Authors/Learners/Raters via `POST /users` (per the `TENANT_USER_ROLES` constant + `GET /users/roles`). Duplicate emails per tenant should return HTTP 409.
