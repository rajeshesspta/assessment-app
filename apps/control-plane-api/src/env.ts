...
const cosmosEnvSchema = commonSchema.extend({
  ...
  CONTROL_PLANE_COSMOS_ENGINE_SIZES_CONTAINER: z.string().min(1),
});
...
export const env = envSchema.parse({
  ...
  CONTROL_PLANE_COSMOS_ENGINE_SIZES_CONTAINER: process.env.CONTROL_PLANE_COSMOS_ENGINE_SIZES_CONTAINER,
});
