...
export function TenantSessionForm({ value, onSave, onClear }: TenantSessionFormProps) {
  ...
  return (
    <form
      className="rounded-2xl bg-white/90 p-6 shadow-xl shadow-slate-200 ring-1 ring-slate-100 backdrop-blur"
      onSubmit={handleSubmit}
    >
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-slate-900">Tenant session</h2>
          <p className="text-sm text-slate-500">Point the portal at your BFF endpoint and identify the learner who is launching attempts.</p>
        </div>
        {value && (
          <button
            type="button"
            onClick={onClear}
            className="inline-flex items-center rounded-full border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 transition hover:border-slate-300 hover:text-slate-800"
          >
            Clear session
          </button>
        )}
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <label className="text-sm font-medium text-slate-600">
          BFF API Base URL
          <input
            className="mt-2 w-full rounded-lg border-slate-200 text-slate-900 shadow-sm focus:border-brand-500 focus:ring-brand-500"
            type="text"
      ...
      <div className="mt-6">
        <span className="text-sm font-semibold text-slate-600">Actor roles</span>
      ...
      <div className="mt-6 flex justify-end">
        <button
          type="submit"
          disabled={!canSubmit}
          className="inline-flex items-center rounded-full bg-brand-600 px-6 py-2 text-sm font-semibold text-white shadow-lg shadow-brand-200 transition hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-60"
        >
          Save session
        </button>
      </div>
